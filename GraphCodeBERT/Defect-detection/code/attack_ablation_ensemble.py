...
def main():
    ...
    for index, example in enumerate(eval_dataset):
        ...
        if is_success == -1:
            attacker.args.num_of_changes = 2
            code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.insert_attack(example, adv_variable, sub)
            if is_success == -1:
                code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.insert_attack(example, adv_variable, sub_2)

        adv_variable_insert = adv_code
        if is_success == -1:
            attacker.args.num_of_changes = 2
            code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.empty_print_attack(example, adv_variable_insert, sub)
            if is_success == -1:
                code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.empty_print_attack(example, adv_variable_insert, sub_2)

        adv_variable_insert_print = adv_code
        if is_success == -1:
            attacker.args.num_of_changes = 2
            code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_if_attack(example, adv_variable_insert_print, sub)
            if is_success == -1:
                code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_if_attack(example, adv_variable_insert_print, sub_2)

        adv_variable_insert_print_if = adv_code
        if is_success == -1:
            attacker.args.num_of_changes = 4
            code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_while_attack(example, adv_variable_insert_print_if, sub)
            if is_success == -1:
                code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_while_attack(example, adv_variable_insert_print_if, sub_2)
    ...
    #--------------------------------------------------------------------------------------------
    with open('./advs_graphcodebert_defectdetection/adv_graphcodebert_ablation_ensemble.txt', 'w') as f:
...
# nohup bash ablation_ensemble.sh >ablation_ensemble.log 2>&1 &