...
class Model_sub(nn.Module):
    ...
    def forward(self, input_ids, attn_mask, position_idx):
        nodes_mask=position_idx.eq(0)
        token_mask=position_idx.ge(2)
...
class Model_for_sim(nn.Module):   
    ...
    def forward(self, inputs_ids=None, attn_mask=None, position_idx=None, labels = None):
        ...
        avg_embeddings=torch.einsum("abc,acd->abd",nodes_to_token_mask,inputs_embeddings)
        inputs_embeddings=inputs_embeddings*(~nodes_mask)[:,:,None]+avg_embeddings*nodes_mask[:,:,None]    
        logits = self.encoder(inputs_embeds=inputs_embeddings,attention_mask=attn_mask,position_ids=position_idx)[0]
        outputs = self.encoder.roberta(inputs_embeds=inputs_embeddings,attention_mask=attn_mask,position_ids=position_idx)[0]
