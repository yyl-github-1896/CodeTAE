...
from utils import is_valid_variable_name, _tokenize, get_identifier_posistions_from_code, get_masked_code_by_position, get_substitutes, is_valid_substitute, map_chromesome
...
class GeneticAlgorithm():
    ...
    def __init__(self, initial_population):
        ...
        self.model_sub = Model_sub(RobertaModel.from_pretrained(args.base_model), args=args).to('cuda')
    ...
    def compute_fitness(self, embedding_target, chromesome):
        ...
        embedding_chromesome = self.model_sub(torch.tensor(input_ids_chromesome).to('cuda')).detach().cpu()
    ...
    def get_substitutes(self, tgt_variable):
        ...
        embedding_target = self.model_sub(torch.tensor(input_ids_target).to('cuda')).detach().cpu()
...
# nohup bash get_subs_gan.sh >get_subs_gan.log 2>&1 &