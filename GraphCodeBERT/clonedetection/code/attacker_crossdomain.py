...
class CrossDomainAttacker():
    ...
    def features_to_model_sub(self, InputFeatures, args):
        input_ids_1 = InputFeatures.input_ids_1
        position_idx_1 = InputFeatures.position_idx_1
        dfg_to_code_1 = InputFeatures.dfg_to_code_1
        dfg_to_dfg_1 = InputFeatures.dfg_to_dfg_1

        input_ids_2 = InputFeatures.input_ids_2
        position_idx_2 = InputFeatures.position_idx_2
        dfg_to_code_2 = InputFeatures.dfg_to_code_2
        dfg_to_dfg_2 = InputFeatures.dfg_to_dfg_2
    ...
    def convert_code_to_features(self, code1, code2, tokenizer, label, args):
        # 这里要被修改..
        feat = []
    ...
    # def compute_fitness(self, chromesome, code_2, codebert_tgt, tokenizer_tgt, orig_prob, orig_label, true_label ,code_1, names_positions_dict, args, orig_example=None, target_example=None):
    #     # 计算fitness function.
    #     # words + chromesome + orig_label + current_prob
    #     temp_replace = map_chromesome(chromesome, code_1, "java")
    #     new_feature = self.convert_code_to_features(temp_replace,
    #                                                 code_2,
    #                                                 self.tokenizer,
    #                                                 true_label,
    #                                                 args)
    #     input_ids_1, position_idx_1, attn_mask_1, input_ids_2, position_idx_2, attn_mask_2 = self.features_to_model_sub(new_feature, self.args)
    #     new_embeddings = self.model_sub(input_ids_1, position_idx_1, attn_mask_1, input_ids_2, position_idx_2, attn_mask_2)
    #     if target_example is not None:
    #         target_embeddings = self.model_sub(target_example[0].unsqueeze(0).to(self.args.device))
    #         fitness_value = -self.loss(target_embeddings, new_embeddings)
    #     else:
    #         orig_embeddings = self.model_sub(orig_example[0], orig_example[1], orig_example[2], orig_example[3], orig_example[4], orig_example[5])
    ...
    def get_importance_score(self, args, example, code, code_2, words_list: list, sub_words: list, variable_names: list, tgt_model, label_list, batch_size=16, max_length=512, model_type='classification', target_example=None):
        ...
        for index, code1_tokens in enumerate([words_list] + masked_token_list):
            new_feature = self.convert_code_to_features(' '.join(code1_tokens), code_2, self.tokenizer, example[6].item(), args)
        ...
        for feature in new_examples:
            ...
            input_ids_1, position_idx_1, attn_mask_1, input_ids_2, position_idx_2, attn_mask_2 = self.features_to_model_sub(feature, self.args)
            embeddings_list.append(self.model_sub(torch.tensor(input_ids_1).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_1).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_1).unsqueeze(0).to(self.args.device), torch.tensor(input_ids_2).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_2).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_2).unsqueeze(0).to(self.args.device)).detach().cpu())   
    ...
    def get_importance_score_explain(self, args, example, code, code_2, words_list: list, sub_words: list, variable_names: list, tgt_model, tokenizer, label_list, batch_size=16, max_length=512, model_type='classification'):
        ...
        for index, code1_tokens in enumerate([words_list] + masked_token_list):
            new_feature = self.convert_code_to_features(' '.join(code1_tokens),code_2,tokenizer, example[6].item(), args)
    ...
    # def ga_attack(self, example, substitutes, code, initial_replace=None, target_example=None):
    #     '''
    #     return
    ...
    #     temp_label = None
    #     orig_embeddings = self.model_sub(example[0].to(self.args.device))
    #     if target_example is not None:
    #         target_embeddings = self.model_sub(target_example[0].to(self.args.device))
    ...
    #                 temp_replace = self.tokenizer.tokenize(temp_replace)
    ...
    #                 replace_embeddings_list.append(self.model_sub(torch.tensor(example).to(self.args.device)).detach().cpu())
    #                 # 3. 将他们转化成features
    ...
    #         if target_example is not None:
    #             temp_fitness = self.compute_fitness(temp_chromesome, words_2, self.model_tgt, self.tokenizer, max(orig_prob), orig_label, true_label ,code_1, names_positions_dict, self.args, orig_example=example, target_example=target_example)
    #         else:
    #             temp_fitness = self.compute_fitness(temp_chromesome, words_2, self.model_tgt, self.tokenizer, max(orig_prob), orig_label, true_label ,code_1, names_positions_dict, self.args, orig_example=example, target_example=None)
    ...
    #             _tmp_mutate_code = self.tokenizer.tokenize(_tmp_mutate_code)
    ...
    #             mutate_embeddings_list.append(self.model_sub(torch.tensor(feature).to(self.args.device)).detach().cpu())
    #         # mutate_embeddings = torch.stack(mutate_embeddings_list)

    #         mutate_fitness_values = []
    #         for index, mutate_embed in enumerate(mutate_embeddings_list):
    #             if self.targeted:
    ...
    #     adv_code = map_chromesome(population[best_index], code_1, "java")
    ...
    # def greedy_attack_baseline(self, example, substitutes, code, target_example=None):
    #     '''
    #     return
    #         original program: code
    #         program length: prog_length
    ...
    def greedy_attack(self, example, substitutes, code, target_example=None):
            ...
            # print(example[0])
            # print(example[0].shape) #torch.Size([1020])
            orig_embeddings = self.model_sub(example[0].unsqueeze(0).to(self.args.device), example[1].unsqueeze(0).to(self.args.device), example[2].unsqueeze(0).to(self.args.device), example[3].unsqueeze(0).to(self.args.device), example[4].unsqueeze(0).to(self.args.device), example[5].unsqueeze(0).to(self.args.device))
            if target_example is not None:
                target_embeddings = self.model_sub(target_example[0].unsqueeze(0).to(self.args.device))
                assert orig_embeddings.shape == target_embeddings.shape
            ...
            for idx, name_and_score in enumerate(sorted_list_of_names):
                ...
                for item in used_substitutes:
                    if item in all_substitutes:
                        all_substitutes.remove(item)
                # print('length of all_substitutes: {}'.format(len(all_substitutes)))
                ...
                for feature in replace_examples:
                    input_ids_1, position_idx_1, attn_mask_1, input_ids_2, position_idx_2, attn_mask_2 = self.features_to_model_sub(feature, self.args)
                    replace_embeddings_list.append(self.model_sub(torch.tensor(input_ids_1).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_1).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_1).unsqueeze(0).to(self.args.device), torch.tensor(input_ids_2).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_2).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_2).unsqueeze(0).to(self.args.device)).detach().cpu())
                    # 3. 将他们转化成features
                # replace_embeddings = torch.stack(replace_embeddings_list)
            ...
            #  query the model_tgt to see whether the attack is success
            temp_adv_code = adv_code
            adv_feature = self.convert_code_to_features(temp_adv_code, 
                                                    code_2,
                                                    self.tokenizer,
                                                    true_label,
                                                    self.args)
            adv_dataset = CodePairDataset([adv_feature], self.args)
            logits, preds = self.model_tgt.get_results(adv_dataset, self.args.eval_batch_size)
            logit, temp_label = logits[0], preds[0]
            if self.targeted:
                target_label = target_example[1]
    ...
    def insert_attack(self, example, substitutes, code, adv=None, target_example=None):
        ...
        # print(example[0])
        # print(example[0].shape) #torch.Size([1020])
        orig_embeddings = self.model_sub(example[0].unsqueeze(0).to(self.args.device), example[1].unsqueeze(0).to(self.args.device), example[2].unsqueeze(0).to(self.args.device), example[3].unsqueeze(0).to(self.args.device), example[4].unsqueeze(0).to(self.args.device), example[5].unsqueeze(0).to(self.args.device))[0]
        if target_example is not None:
            target_embeddings = self.model_sub(target_example[0].unsqueeze(0).to(self.args.device))
            assert orig_embeddings.shape == target_embeddings.shape
        ...
        for i in range(insert_pos_nums):
            ...
            new_feature = self.convert_code_to_features(masked_code, 
                                                    code_2,
                                                    self.tokenizer,
                                                    true_label,
                                                    self.args)
            input_ids_1, position_idx_1, attn_mask_1, input_ids_2, position_idx_2, attn_mask_2 = self.features_to_model_sub(new_feature, self.args)
            masked_embeddings = self.model_sub(torch.tensor(input_ids_1).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_1).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_1).unsqueeze(0).to(self.args.device), torch.tensor(input_ids_2).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_2).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_2).unsqueeze(0).to(self.args.device))[0].detach().cpu()
        ...
        for i, insert_idx in enumerate(sorted_id):
            ...
            for sub in subs_for_insert:
                ...
                adv_feature = self.convert_code_to_features(adv_code, 
                                                        ...
                                                        true_label, 
                                                        self.args)
                input_ids_1, position_idx_1, attn_mask_1, input_ids_2, position_idx_2, attn_mask_2 = self.features_to_model_sub(adv_feature, self.args)
                adv_embeddings = self.model_sub(torch.tensor(input_ids_1).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_1).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_1).unsqueeze(0).to(self.args.device), torch.tensor(input_ids_2).unsqueeze(0).to(self.args.device), torch.tensor(position_idx_2).unsqueeze(0).to(self.args.device), torch.tensor(attn_mask_2).unsqueeze(0).to(self.args.device))[0].detach().cpu()
