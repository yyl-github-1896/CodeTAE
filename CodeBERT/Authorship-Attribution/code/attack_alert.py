...
def main():
    ...
    try:
        ...
        config = config_class.from_pretrained('../cache/microsoft/codebert-base/config.json')
    ...
    try:
        ...
        tokenizer = tokenizer_class.from_pretrained('../cache/microsoft/codebert-base/')
    ...
    if args.model_name_or_path:
        try:
            ...
            model_tgt = model_class.from_pretrained('../cache/microsoft/codebert-base/')
    else:
    ...
    try:
        ...
        model_sub = RobertaModel.from_pretrained('../cache/microsoft/codebert-base/').to(args.device)
    ...
    codes_file_path = os.path.join(folder, '{}_subs_baseline.jsonl'.format(
                                file_type))
    ...
    for index, (example_eval) in enumerate(eval_dataset):
        ...
        subs = substs[index]
        code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.greedy_attack_baseline(example, code, subs)
    ...
    with open('adv_codebert_baseline_untargeted.txt', 'w') as f:
