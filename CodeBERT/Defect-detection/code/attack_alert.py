...
def main():
    ...
    for index, example in enumerate(eval_dataset):
        ...
        code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.greedy_attack_baseline(example, code, substituions)
        attack_type = "Greedy"
        if is_success == -1:
            # 如果不成功，则使用gi_attack
    ...
    #--------------------------------------------------------------------------------------------
    with open('adv_codebert_alert_untargeted.txt', 'w') as f:
