...
class CrossDomainAttacker():
    ...
    def compute_fitness(self, chromesome, words_2, codebert_tgt, tokenizer_tgt, orig_prob, orig_label, true_label ,code, names_positions_dict, args, orig_example=None, target_example=None):
        ...
        new_feature = convert_examples_to_features(temp_code, 
                                                    ...
                                                    self.tokenizer,
                                                    args, None).input_ids
        new_embeddings = self.model_sub(torch.tensor(new_feature).to(self.args.device))
        if target_example is not None:
            ...
            orig_embeddings = self.model_sub(orig_example[0].to(self.args.device))
    ...
    def get_importance_score(self, args, example, code, code_2, words_list: list, sub_words: list, variable_names: list, tgt_model, label_list, batch_size=16, max_length=512, model_type='classification', target_example=None):
        ...
        for index, code1_tokens in enumerate([words_list] + masked_token_list):
            new_feature = convert_examples_to_features(code1_tokens,code2_tokens,example[1].item(),None, None,self.tokenizer,args, None).input_ids
        ...
        for example in new_examples:
            ...
            embeddings_list.append(self.model_sub(torch.tensor(example).to(self.args.device)).detach().cpu())   
    ...
    def greedy_attack(self, example, substitutes, code, target_example=None):
            ...
            # print(example[0].shape) #torch.Size([1020])
            orig_embeddings = self.model_sub(example[0].to(self.args.device))
            # print('orig_embeddings.shape: ', orig_embeddings.shape)
    ...
    def insert_attack(self, example, substitutes, code, adv=None, target_example=None):
        ...
        for i in range(insert_pos_nums):
            ...
            processed_masked_code = self.tokenizer.tokenize(processed_masked_code)
            new_feature = convert_examples_to_features(processed_masked_code, 
                                                    words_2,
                                                    true_label, 
                                                    None, None,
                                                    self.tokenizer,
                                                    self.args, None).input_ids
            masked_embeddings = self.model_sub(torch.tensor(new_feature).to(self.args.device)).detach().cpu()
        ...
        for i, insert_idx in enumerate(sorted_id):
            ...
            for sub in subs_for_insert:
                ...
                processed_adv_code = self.tokenizer.tokenize(processed_adv_code)
                adv_feature = convert_examples_to_features(processed_adv_code, 
                                                        words_2,
                                                        true_label, 
                                                        None, None,
                                                        self.tokenizer,
                                                        self.args, None).input_ids
                adv_embeddings = self.model_sub(torch.tensor(adv_feature).to(self.args.device)).detach().cpu()
